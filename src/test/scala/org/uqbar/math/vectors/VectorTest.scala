...
class VectorTest extends FreeSpec with Matchers {
  ...
  implicit val VectorEquality = new Equality[Vector] {
    def areEqual(v: Vector, o: Any) = {
      o match {
        case v2: Vector             => DoubleEquality.areEqual(v.x, v2.x) && DoubleEquality.areEqual(v.y, v2.y)
        case (x: Number, y: Number) => DoubleEquality.areEqual(v.x, x) && DoubleEquality.areEqual(v.y, y)
        case _                      => false
      }
    }
  }
